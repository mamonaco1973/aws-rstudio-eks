...
data "aws_vpc" "k8s-vpc" {
  ...
  filter {
    ...
    values = ["eks-vpc"]       # Match the tag value for the desired VPC.
  }
}
...
data "aws_subnet" "k8s-subnet-1" {
  ...
  filter {
    ...
    values = ["pub-subnet-1"]          # Match the tag value for the desired subnet.
  }
}
...
data "aws_subnet" "k8s-subnet-2" {
  ...
  filter {
    name = "tag:Name"                  # Use the "Name" tag to identify the subnet.
    values = ["pub-subnet-2"]         # Match the tag value for the desired subnet.
  }
}
...
data "aws_subnet" "k8s-private-subnet-1" {
  ...
  filter {
    name   = "tag:Name"                     # Use the "Name" tag to identify the subnet.
    values = ["priv-subnet-1"]       # Match the tag value for the desired private subnet.
  }
}
...
data "aws_subnet" "k8s-private-subnet-2" {
  ...
  filter {
    name   = "tag:Name"                     # Use the "Name" tag to identify the subnet.
    values = ["priv-subnet-2"]       # Match the tag value for the desired private subnet.
  }
}